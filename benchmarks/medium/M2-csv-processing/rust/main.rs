...
fn generate_csv() {
    let f = fs::File::create(CSV_FILE).unwrap();
    let mut w = BufWriter::with_capacity(1 << 20, f);
    writeln!(w, "id,first_name,last_name,email,department,salary").unwrap();
    let mut state: u32 = 42;
    for i in 0..N {
        state = state.wrapping_mul(1664525).wrapping_add(1013904223);
        let salary = 30000.0 + (state as f64 / 4294967295.0) * 120000.0;
        let first = FIRST_NAMES[i % 10];
        let last = LAST_NAMES[i % 10];
...
fn process() {
    let f = fs::File::open(CSV_FILE).unwrap();
    let reader = BufReader::with_capacity(1 << 20, f);
    let mut groups: HashMap<String, DeptStats> = HashMap::with_capacity(8);
    ...
    for line in lines {
        ...
        let salary: f64 = line[last_comma + 1..].parse().unwrap();
        if salary > 75000.0 {
            let prev_comma = line[..last_comma].rfind(',').unwrap();
            let dept = &line[prev_comma + 1..last_comma];
            let entry = groups.entry(dept.to_string()).or_insert(DeptStats {
                count: 0,
                total: 0.0,
    ...
    let mut results: Vec<(String, usize, f64, f64, f64)> = groups
        .into_iter()
        .map(|(dept, g)| (dept, g.count, g.total / g.count as f64, g.max, g.min))
        .collect();
    ...
    writeln!(w, "department,count,avg_salary,max_salary,min_salary").unwrap();
    for (dept, count, avg, max, min) in &results {
        writeln!(w, "{},{},{:.2},{:.2},{:.2}", dept, count, avg, max, min).unwrap();
    }
}
...
fn main() {
    ...
    println!("=== BENCHMARK: M2 - CSV Processing ===");
    ...
    fs::remove_file(CSV_FILE).unwrap();
